...
(function ( $ ) {
    	...
    	this.each(function(){
	        ...
	        if(originalSelect[0].tagName.toUpperCase()==="SELECT"){
	        	...
	        	var inputSelect = $("<input/>").insertBefore(originalSelect);
					...
					var objName = originalSelect.attr("name");
	        	...
	        	inputSelect.attr({
	        		...
	        		id: objID,
					'data-count': originalSelect.attr("data-count"),
					'data-sid':-1
	        	});
    ...
    function prepareOption(value, wrapper){
    	var selectOption = $("<li data-sid='-1'>"+value+"</li>").appendTo(wrapper.children("ol"));
    	...
    	selectOption.css({
   		...
   		selectOption.click(function(){
			...
			inputSelect.attr('data-sid',selectOption.data('sid'));
   			inputSelect.trigger("change");
   		});	
    }
	function insertOption(elem, wrapper){
		var selectOption = $("<li data-info='"+elem.text()+"' data-sid='"+elem.data('sid')+"'>"+elem.text()+"</li>").appendTo(wrapper.children("ol"));
		var inputSelect = wrapper.children("input");
		selectOption.css({
			padding: "3px",
			textAlign: "left",
		...
		selectOption.click(function(){
			inputSelect.val(selectOption.text());
			inputSelect.attr('data-sid',selectOption.data('sid'));
			inputSelect.attr('data-info',selectOption.data('info'));
			inputSelect.trigger("change");
		});
	}
